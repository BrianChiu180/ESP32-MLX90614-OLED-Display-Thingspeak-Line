...
WiFiClientSecure client;                                                                     //Http connect WIFI library
...
void setup() 
{
  ...
  Serial.println(WiFi.localIP()); //Display WIFI IP
...
{
  display.clearDisplay();
  display.setRotation(2);                                                                   //OLED can disaply rolocate location and decide by user
  display.setTextSize(2);                                                                   // Set Text size
  display.setTextColor(1);                                                                  // Set 1:OLED default color
...
void get_surface()
{
    ...
    Wire.requestFrom(0x5A, 3);                                                                             //Read 3 dtata from I2C slave address (The data are only save on buffer, not feedback to MLX90614)
...
//    message += "\nTemperature=" + String((int)mlx.readObjectTempC()) + " *C";           //The message content of Line
   message += "\nTemperature=" + String((int)Objtemp1) + " *C";           //The message content of Line
    ...
    //if (client.connect("notify-api.line.me", 443))
    {
      ...
      String url1 = "/api/notify";                                                       //POST header
      client.println("POST " + url1 + " HTTP/1.1");                                      //POST header
      client.print("Host: "); client.println(host);                                      //POST header
      
      client.print("Authorization: Bearer "); client.println(Linetoken);                //Scepter
      client.println("Content-Type: application/x-www-form-urlencoded");                //Scepter
      client.print("Content-Length: "); client.println( String((LEN + 8)) );            //Scepter
      client.println();                                                                 //Scepter
      client.print("message="); client.println(message);                                //Scepter
      client.println();
      
      delay(1000);                                                                        //Delay 1 sec 
      String response = client.readString();

      Serial.println(response);                                                         //Disaply the result of send to Line
